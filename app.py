...
def list_user(user_id):
	...
	print("Opened database successfully")
	cursor = conn.cursor()
	cursor.execute("SELECT username, full_name, emailid, password, id from users where id=?",(user_id,))
...
def list_tweet(user_id):
	...
	print ("Opened database successfully")
	cursor = conn.cursor()
	cursor.execute("SELECT id, username, body, tweet_time from tweets where id=?",(user_id,))
	data=cursor.fetchall()
	print(data)
	if len(data)==0:
		abort(404)
